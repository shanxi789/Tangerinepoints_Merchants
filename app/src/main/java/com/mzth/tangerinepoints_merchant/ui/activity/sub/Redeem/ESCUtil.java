...
public class ESCUtil {
	...
	// --------------------
	public static byte[] generateMockData(InvoiceBean bean) {
		try {
			byte[] next2Line = ESCUtil.nextLine(2);
			byte[] title = ("TangerinePoints Merchant Receipt").getBytes("gb2312");
			byte[] fontSize2Big = ESCUtil.fontSizeSetBig(2);
			byte[] center = ESCUtil.alignCenter();

			byte[] left = ESCUtil.alignLeft();
			byte[] orderSerinum = "Transaction Type:".getBytes("gb2312");
			byte[] nextLine = ESCUtil.nextLine(1);
			byte[] type = bean.getTxnType().getBytes("gb2312");
			...
			byte[] priceInfo = "Transaction ID:".getBytes("gb2312");
			nextLine = ESCUtil.nextLine(1);
			byte[] id = bean.getTxnId().getBytes("gb2312");
			center = ESCUtil.alignCenter();
			nextLine = ESCUtil.nextLine(1);

			byte[] priceShouldPay = ("Item:"+bean.getItem()).getBytes("gb2312");
			nextLine = ESCUtil.nextLine(1);
			//时间的转换
			long time = bean.getTime();
			SimpleDateFormat format = new SimpleDateFormat("MM/dd/yyyy");
			Date date = new Date(time);
			byte[] takeTime = ("Time:"+format.format(date)).getBytes("gb2312");
			nextLine = ESCUtil.nextLine(1);

			byte[] setOrderTime = ("Store:"+bean.getBusinessName()).getBytes("gb2312");
			nextLine = ESCUtil.nextLine(1);
			byte[] setLocation = "Location:".getBytes("gb2312");
			nextLine = ESCUtil.nextLine(1);
			byte[] setAddress = bean.getBusinessLocation().getBytes("gb2312");
			center = ESCUtil.alignCenter();
...
//			byte[] tips_2 = "饭后点评再奖5毛".getBytes("gb2312");
			...
			byte[][] cmdBytes = {
					center,fontSize2Big,title, next2Line,left,fontSize1Small,
					orderSerinum, type,nextLine,priceInfo,id,nextLine,left,priceShouldPay,
					nextLine, left,takeTime, nextLine, left,setOrderTime,
					nextLine,left,setLocation,setAddress,next4Line,breakPartial};
